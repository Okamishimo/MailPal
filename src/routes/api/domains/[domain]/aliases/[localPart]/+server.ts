...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
	...
	if (enabled !== undefined) {
		if (typeof enabled !== 'boolean') return json({ error: 'enabled must be a boolean' }, { status: 400 });
		updated.enabled = enabled;
	}
	if (targetEmail !== undefined) {
		if (targetEmail !== null && typeof targetEmail !== 'string') {
			return json({ error: 'targetEmail must be a string or null' }, { status: 400 });
		}
		updated.targetEmail = targetEmail === '' ? null : targetEmail;
	}
	if (note !== undefined) {
		if (note !== null && typeof note !== 'string') return json({ error: 'note must be a string or null' }, { status: 400 });
	...
	if (expiresAt !== undefined) {
		if (expiresAt !== null && (typeof expiresAt !== 'number' || !Number.isFinite(expiresAt))) {
			return json({ error: 'expiresAt must be a number or null' }, { status: 400 });
		}
	...
	if (maxForwards !== undefined) {
		if (maxForwards !== null && (typeof maxForwards !== 'number' || !Number.isInteger(maxForwards) || maxForwards < 1)) {
			return json({ error: 'maxForwards must be a positive integer or null' }, { status: 400 });
		}
